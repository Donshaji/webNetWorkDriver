...
2. **TLS Handshake**: 
    - **ClientHello**: The client sends a message listing supported cipher suites and TLS versions.
    - **ServerHello**: The server responds with the chosen cipher suite and its digital certificate (containing the public key).
    - **Certificate Validation**: The client verifies the server's certificate against trusted Root Certificate Authorities.
...
3. **Encrypted Data Transfer**: All subsequent HTTP traffic (GET/POST requests, headers, and body) is encrypted symmetrically.
